...
http.createServer(function(req, res) {
	...
	if(req.url == '/upload' && req.method.toLowerCase() == 'post') {
		...
		var fileWrap = nowYear + '-' + nowMonth + '-' + nowDay;
		var fileName = d.getTime();
		var filetime= fileWrap;
		...
		mkdirs(fileWrap, function() {
			...
			form.parse(req, function(err, fields, files) {
				 ...
				 res.end('<a href="javascript:void(0)" onclick="window.history.back();">点击此处返回</a>');
				...
				// 	divv.text('Time:'+filetime);
				// 	var divahref = $('<a></a>');
				// 	var _name = path.basename(result.upload.path);
				// 	divahref.attr('href', fileWrap.slice(6) +'/'+_name);
...
//   files:{ upload:File {
				...
				//         size: 21469,
				//         path: 'D:\\git\\learnasdasding-node\\uploadFile\\public\\upload\\2017-4-2\\upload_24bc0e6c603d412356cbfe29fdb56084.gif',
				//         name: '201103040950491866.gif',
				//         type: 'imadsadage/gif',
				//         haasdasdsh: null,
				//         lastModifiedDate: 2017-03-04T09:39:01.000Z,
				//         _writeStream: [Objecasdasdt]sdadsad } } }